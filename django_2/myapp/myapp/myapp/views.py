...
from myapp.forms import priceForm,orderForm,MyRegistrationForm,foodForm,restaurantForm,areaForm,branchForm,menuForm,employeeForm,update_branch,update_menu,update_employee
...
def delete_item(request,table_name):
	...
	if request.method=='POST':
		...
		for item in delete_list:
			if table_name.upper()=="AREA":
				c.execute("delete from employee where area_id=%s",(item,))
				c.execute("select branch_id from branch where area_id=%s",(item,))
				branch_list=[]
				b=c.fetchall()
				for ii in b:
					print(ii)
					c.execute("delete from menu where branch_id=%s",(ii[0],))
					c.execute("delete from contact_branch where branch_id=%s",(ii[0],))
				c.execute("delete from branch where area_id=%s",(item,))
				c.execute("delete from AREA where area_id=%s",(item,))
				success="success"
			elif table_name.upper()=="RESTAURANT":
				c.execute("select branch_id from branch where restaurant_id=%s",(item,))
				...
				for ii in b:
					...
					c.execute("delete from menu where branch_id=%s",(ii[0],))
					c.execute("delete from contact_branch where branch_id=%s",(ii[0],))
				c.execute("delete from branch where restaurant_id=%s",(item,))
				...
				c.execute("delete from menu where branch_id=%s",(item,))
				c.execute("delete from contact_branch where branch_id=%s",(item,))
	...
	return render(request,"delete_item.html",RequestContext(request,{'table_name':table_name,'column_list':column_list,'items':items,'success':success}))
...
def update_item(request,table_name):
	...
	return render(request,"update_item.html",RequestContext(request,{'table_name':table_name,'column_list':column_list,'items':items,'form':form}))
